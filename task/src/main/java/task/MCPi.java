...
public class MCPi implements DistTask
{
	...
	// Implementation of the task.DistTask interface.
	//  Called at the worker side to perform the computations.
	public void compute()
	{ 
		System.out.println("task.DistTask: compute : started");
		pi = calcPi(samples);
		System.out.println("task.DistTask: compute : completed");
	}

	private double calcPi(long n)
	{
		System.out.println("task.DistTask: calcPi : number of sample : " + n);
		...
		System.out.println("task.DistTask: calcPi : computed value of pi : " + pival);
		return pival;
	}
	...
	public static void main(String args[])
	{
		...
		task.MCPi mcpi = new task.MCPi(n);
		mcpi.compute();
